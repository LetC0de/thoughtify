...
class Thoughtify {
  ...
  cacheDOMElements() {
    ...
    this.loginPwToggle = document.getElementById('loginPwToggle');
    ...
    this.regPwToggle = document.getElementById('regPwToggle');
  ...
  bindEvents() {
    ...
    if (this.loginPwToggle) {
      this.loginPwToggle.addEventListener('click', () => this.toggleLoginPassword());
    }
    if (this.regPwToggle) {
      this.regPwToggle.addEventListener('click', () => this.toggleRegPassword());
    }
  ...
  setAuthLoading(form, loading) {
    if (form === 'login') {
      ...
      this.loginBtnText.textContent = loading ? 'Logging in...' : 'Log In';
    } else {
  ...
  async fetchPublicFeed() {
    ...
    try {
      ...
      const res = await fetch(`${this.apiBaseThought}/public`, { headers });
      ...
      <div class="thought-card-footer">
        <div class="card-footer-actions">
          <button class="card-action-btn card-like-btn ${liked ? 'liked' : ''}" data-liked="${liked}" data-thought-id="${thought.id}" aria-label="Like">
            <svg class="like-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
...
Thoughtify.prototype.toggleLoginPassword = function() {
  const input = this.loginPassword;
  if (!input) return;
  const isPassword = input.type === 'password';
  input.type = isPassword ? 'text' : 'password';
  this.loginPwToggle.innerHTML = isPassword
    ? '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>'
    : '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
};
...
Thoughtify.prototype.toggleRegPassword = function() {
  const input = this.regPassword;
  if (!input) return;
  const isPassword = input.type === 'password';
  input.type = isPassword ? 'text' : 'password';
